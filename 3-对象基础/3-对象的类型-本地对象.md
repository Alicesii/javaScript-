...
#### 1.6.push()方法：用于在Array类结尾添加一个或多个项。

#### 1.7.pop()方法：用于删除最后一个数组项，数组的最后一项总是[length-1]。
...
#### 1.8.shift()：删除数组中的第一个项，将其作为函数的返回值。

#### 1.9.unshfit()：在数组的头部插入一个元素。
...
#### 1.10.reverse():颠倒数组项的顺序。

var arr=[1,2,3,4];

arr.reverse();   //[4, 3, 2, 1]

#### 1.11.sort()：根据数组项的值按升序为它们排序。
...
#### 1.12.splice()方法：插入、删除或替换一个数组的部分元素。
